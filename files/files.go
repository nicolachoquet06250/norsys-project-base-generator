...
func (f File) Create(path string, content string) error {
	...
	if !Exists(_path) {
		return fmt.Errorf("%s directory not exists", _path)
	}
	...
	var _f, cErr = os.Create(path)
	if cErr != nil {
		return fmt.Errorf("%s file open error", cErr.Error())
	}

	var _, wErr = _f.Write([]byte(content))
	if wErr != nil {
...
func (f Dir) Create(path string, content string) error {
	path = strings.ReplaceAll(path, Slash()+Slash(), Slash())
